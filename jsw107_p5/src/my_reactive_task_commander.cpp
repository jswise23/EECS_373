//FILE ADAPTED FOR USE FROM irb120_reactive_task_commander, provided by Dr. Newman part 5 files
...
int main(int argc, char** argv) {
    ...
    flange_origin << g_perceived_object_pose.pose.position.x, g_perceived_object_pose.pose.position.y, 0.2; //specify coordinates for the desired flange position (origin) with respect to the robot's base frame
    ...
    //go to pose just to the side of the gear part, along the ground
    flange_origin <<g_perceived_object_pose.pose.position.x * .8, g_perceived_object_pose.pose.position.y *.8 , 0.001; 
    ...
    //push gear part directly outward for a distance
    flange_origin << g_perceived_object_pose.pose.position.x * 1.2, g_perceived_object_pose.pose.position.y * 1.2, 0.001;
    goal_flange_affine.translation() = flange_origin;
    ROS_INFO_STREAM("move to flange origin= " << goal_flange_affine.translation().transpose() << endl);
    ...
    //Lift up and finish
    flange_origin << g_perceived_object_pose.pose.position.x, g_perceived_object_pose.pose.position.y, 0.2;
    goal_flange_affine.translation() = flange_origin;
    ROS_INFO_STREAM("move to flange origin= " << goal_flange_affine.translation().transpose() << endl);
    g_q_vec_arm_Xd = optimal_path.back(); //start from the joint-space pose that ended the prior plan
    //convert move to an optimal joint-space path:

...
void coordCB(const geometry_msgs::PoseStamped& message_holder) {
    xBlock = message_holder.pose.position.x;
    yBlock = message_holder.pose.position.y;
    blockFound = true;
}
...
int main(int argc, char** argv) {
    ...
    //subscribe to gete block coordinate
    ros::Subscriber coord_subscriber = nh.subscribe("block_pose", 1, coordCB);
    ...
    while (ros::ok()) {
        ...
        //find block position
        while(!blockFound){
            ROS_INFO("Waiting to recieve block position...");
            ros::spinOnce();
        ...
        tool_pose.pose.position.x = xBlock;
        tool_pose.pose.position.y = yBlock; 

...
int main(int argc, char** argv) {
    ...
    if (boxInspector.get_bad_part_Q1(current_part)) {
        ...
        if(ans == 1){
            for(int i = 0; i < nparts; i++){
                geometry_msgs::Pose orphanPose = orphan_models_wrt_world[i].pose;
                //if location of the orphaned part is within a margin of error of the current part, this is the one we want to remove
                if(abs(orphanPose.position.x - current_part.pose.pose.position.x) < marginError && abs(orphanPose.position.y - current_part.pose.pose.position.y) < marginError && abs(orphanPose.position.z - current_part.pose.pose.position.z) < marginError){
